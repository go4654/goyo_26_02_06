...
import { newsUpdateAction } from "./server/news-update.action";
...
export const action = newsUpdateAction;

type ActionResponse = { success: true; slug: string } | { error: string };

/**
 * 뉴스 수정 페이지
 * slug로 조회, 카테고리 enum select, 썸네일/커버 미리보기, 성공 시 /admin/news
 */
export default function NewsEdit({ loaderData }: Route.ComponentProps) {
  ...
  const [newsCategory, setNewsCategory] = useState<NewsCategory>(
    NEWS_CATEGORIES.includes(news.category as NewsCategory)
      ? (news.category as NewsCategory)
      : "notice",
  );
  const [coverPreview, setCoverPreview] = useState<string | null>(
    news.cover_image_urls?.[0] ?? null,
  );
  ...
  const handleCoverRemove = () => {
    ...
    setCoverPreview(news.cover_image_urls?.[0] ?? null);
    setCoverFile(null);
  ...
  const initialFormData: ContentFormData = {
    ...
    thumbnailImageUrl: news.thumbnail_image_url ?? undefined,
    visibility: news.visibility === "member" ? "member" : "public",
  };
