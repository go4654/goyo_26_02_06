...
/** 비밀번호 변경 시 Supabase에서 오는 영문 에러 메시지를 한글로 매핑 */
...
function getNewPasswordErrorMessage(enMessage: string): string {
  return NEW_PASSWORD_ERROR_MESSAGES[enMessage] ?? enMessage;
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (updateError) {
    return data(
      { error: getNewPasswordErrorMessage(updateError.message) },
      { status: 400 },
    );
  }

  // 비밀번호 업데이트 성공 시 로그아웃 후 로그인 페이지로 리다이렉트
  await client.auth.signOut();

  return redirect("/login?passwordUpdated=1", { headers });
}
        ...
        <CardContent className="grid gap-4">
          ...
          >
            ...
            {actionData && "error" in actionData && actionData.error ? (
              <FormErrors
                errors={[getNewPasswordErrorMessage(String(actionData.error))]}
              />
            ) : null}
