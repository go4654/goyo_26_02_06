...
export function ThreeColumns({ children }: { children: ReactNode }) {
  return <div className="mt-10 grid gap-3 md:grid-cols-3">{children}</div>;
}

function Item({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="[&_p]:text-text-2 rounded-xl border bg-gray-100 p-6 backdrop-blur-sm dark:border-white/10 dark:bg-white/5 [&_p]:text-sm [&_p]:leading-relaxed">
      ...
      <div className="text-text-2 text-body leading-relaxed">{children}</div>
    </div>
  );
}
