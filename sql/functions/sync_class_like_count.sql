/**
 * 데이터베이스 트리거 함수: sync_class_like_count
 * 
 * 이 함수는 좋아요가 삽입되거나 삭제될 때 classes 테이블의 like_count를
 * 자동으로 증가/감소시켜 유지합니다.
 * 
 * 두 가지 시나리오를 처리합니다:
 * 1. INSERT: 좋아요가 추가될 때 카운트 증가
 * 2. DELETE: 좋아요가 제거될 때 카운트 감소
 * 
 * 보안 고려사항:
