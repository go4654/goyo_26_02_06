-- =============================================================================
-- get_admin_dashboard_stats() traffic에 today_views 추가
-- - 오늘(UTC 당일) class_view_events + gallery_view_events 건수
-- - 기존 total_views, gallery_views 및 나머지 영역 유지
-- =============================================================================
...
BEGIN
  ...
  -- traffic: 전체 조회수, 갤러리 조회수, 오늘 방문 수(UTC 당일 클래스+갤러리 뷰 이벤트)
  SELECT
    ...
    + (SELECT count(*)::bigint FROM public.gallery_view_events
       WHERE created_at >= (current_date AT TIME ZONE 'UTC')::timestamptz)
  INTO v_total_views, v_gallery_views, v_today_views;
